        ...
        <div class="input_all_wrapper">
            ...
            <?php if ($info->TYPE == 'Y') { ?>
                <div class="label_time pb15">
                    <h4><strong>동영상첨부</strong><span style="font-size:11px;"> 동영상은 100MB이하만 가능합니다.(mp4, avi, wmv, mpg, mpeg, mov)</span></h4>
                    <ul>
                        <li><input type="file" name="video" accept="file_extension|video/*" onchange="chk_file_type_video(this); chk_file_size(this);"></li>
                    </ul>
                </div>
            <?php } ?>
...
<?php if ($info->TYPE == 'N' || $info->TYPE == 'I') { ?>
                                            var f2 = confirm("무료회원은 내기 결과 입력 후 7일이 지나서 다음 내기등록이 가능합니다.");
                                            if (f2) {
...
<?php } ?>
                                ...
                                format: 'Y-m-d H:i',
                                        minDate: dateToday,
                                        onSelectDate: chkDate2,
                                        onSelectTime: chkDate2
                                        // 쿼리에서 권한값을 가져와야함
<?php if ($info->TYPE == 'N') { ?>
...
<?php } ?>
                                    ...
                                    format: 'Y-m-d H:i',
                                    minDate: dateToday,
                                    onSelectDate: chkDate,
                                    onSelectTime: chkDate
                                });
                                if ($("#si").select().val()) {
...
// 카테고리 셀렉트 부분
                            ...
                            function chkDate() {
                                if (getTimeStamp() > $(".option_time").val()) {
                                    alert("현재시간보다 작습니다.");
                                    $(".option_time").val('');
                                    return false;
                                }
